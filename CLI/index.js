...
if (AppState.mode !== 'settings' && AppState.mode !== 'ignore') {
    ...
    if (!AppState.shsf_key) {
        ...
        console.error(chalk.yellow('Run with --mode settings to configure your token, or use --key <token>'));
        process.exit(1);
    }
    ...
    console.log(chalk.gray(`Using SHSF Key: ${AppState.shsf_key.substring(0, 8)}...`));
}
...
async function handleSettings() {
    ...
    try {
        ...
        if (choice === '1') {
            ...
            if (config.token) {
                console.log(chalk.white('Session Token: ') + chalk.green(`${config.token.substring(0, 16)}...`));
            ...
            if (config.token) {
                console.log(chalk.gray(`Current token: ${config.token.substring(0, 16)}...\n`));
            }

            const newToken = await question(rl, chalk.yellow('Enter new session token (or press Enter to cancel): '));
...
async function handleIgnoreMode() {
    if (!AppState.project) {
        console.error(chalk.red('Error: --project parameter is required for ignore mode'));
        process.exit(1);
    }

    if (!fsSync.existsSync(AppState.project)) {
        console.error(chalk.red(`Error: Project folder '${AppState.project}' does not exist.`));
        process.exit(1);
    }
    ...
    if (!opts.file) {
        console.error(chalk.red('Error: --file parameter is required to add/remove patterns'));
        console.log(chalk.yellow('\nUsage:'));
        console.log(chalk.white('  Add pattern:    ') + chalk.cyan('--mode ignore --project <path> --file <pattern>'));
        console.log(chalk.white('  Remove pattern: ') + chalk.cyan('--mode ignore --project <path> --file <pattern> --remove'));
        console.log(chalk.white('  List patterns:  ') + chalk.cyan('--mode ignore --project <path> --list'));
        console.log(chalk.gray('\nExamples:'));
...
async function pushMetadata() {
    ...
    if (typeof localTags === 'string') {
        ...
        localTags = localTags ? localTags.split(',').map(t => t.trim()).filter(t => t) : [];
    ...
    if (typeof remoteTags === 'string') {
        remoteTags = remoteTags ? remoteTags.split(',').map(t => t.trim()).filter(t => t) : [];
...
async function pullFiles() {
    ...
    if (data.status === 'OK') {
        ...
        for (const existingFile of existingFiles) {
            if (!remoteFileNames.includes(existingFile) &&
                !existingFile.startsWith('.')) {
                await fs.unlink(path.join(AppState.project, existingFile));
                console.log(chalk.yellow(`✓ Deleted file: ${existingFile} (not on remote)`));
            }
        }
    }
}
...
async function pullEnv() {
    ...
    if (data.status === 'OK' && data.data.env) {
        ...
        const envContent = env.map(item => `${item.name}=${item.value}`).join('\n') + '\n';
...
async function pushEnv() {
    ...
    const env = lines.map(line => {
        const [name, ...valueParts] = line.split('=');
        const value = valueParts.join('=');
        return { name: name.trim(), value: value.trim() };
    }).filter(item =>
        item.name.length >= 1 && item.name.length <= 128 &&
        item.value.length >= 1 && item.value.length <= 256
    );
...
async function pushFiles() {
    ...
    for (const filename of localFiles) {
        ...
        try {
            ...
            if (!validation.valid) {
                console.log(chalk.yellow(`⚠ Skipping ${filename} (${validation.type}): ${validation.message}`));
                continue;
            }
    ...
    for (const remoteFile of remoteFiles) {
        if (!localFiles.includes(remoteFile.name)) {
            try {
                await apiRequest('DELETE', `/function/${AppState.link}/file/${remoteFile.id}`);
...
async function handleSetUrl() {
    if (!opts.url) {
        console.error(chalk.red('Error: --url parameter is required for set-url mode'));
        process.exit(1);
    }
...
async function main() {
    ...
    if (AppState.mode === 'set-key') {
        if (!opts.key) {
            console.error(chalk.red('Error: --key parameter is required for set-key mode'));
            process.exit(1);
        }
        ...
        const handleChange = async (filePath) => {
            ...
            const timeoutId = setTimeout(async () => {
                ...
                try {
                    ...
                    const validation = await validateFileBeforeSync(filePath, filename);
                    if (!validation.valid) {
                        console.log(chalk.yellow(`⚠ Skipping ${filename} (incomplete or invalid): ${validation.message}`));
                        return;
                    }
