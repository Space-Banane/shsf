...
export async function createGuestUser(params: {
  ...
  return await response.json() as OKResponse<{ guest: GuestUser }> | ErrorResponse;
}
...
export async function assignFunctionToGuest(guestId: number, functionId: number) {
...
export async function unassignFunctionFromGuest(guestId: number, functionId: number) {
  const response = await fetch(`${BASE_URL}/api/account/guest/unassign-function`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ guestId, functionId }),
...
export async function listFunctionGuests(functionId: number): Promise<ListFunctionGuestsResult> {
    const response = await fetch(`${BASE_URL}/api/account/guest/function/${functionId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    });
