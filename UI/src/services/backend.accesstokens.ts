...
export async function generateAccessToken(
    ...
    if (!response.ok) {
        ...
        throw new Error(error.message || `Failed to generate access token: ${response.status}`);
    }
...
export async function revokeAccessToken(id: number) {
    ...
    if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(error.message || `Failed to revoke access token (status: ${response.status})`);
    }
    return await response.json();
}
...
export async function listAccessTokens() {
    ...
    if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(error.message || `Failed to list access tokens (status: ${response.status})`);
    }
...
export async function updateAccessToken(
    ...
    if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(error.message || `Failed to update access token (status: ${response.status})`);
    }
    return await response.json();
}
