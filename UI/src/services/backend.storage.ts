...
export async function deleteStorage(storageName: string) {
  const response = await fetch(`${BASE_URL}/api/storage/${encodeURIComponent(storageName)}`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
  });
...
export async function clearStorageItems(storageName: string) {
  const response = await fetch(`${BASE_URL}/api/storage/${encodeURIComponent(storageName)}/items`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
  });
...
export async function setStorageItem(storageName: string, item: { key: string; value: any; expiresAt?: string | number }) {
  const response = await fetch(`${BASE_URL}/api/storage/${encodeURIComponent(storageName)}/item`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
...
export async function getStorageItem(storageName: string, key: string) {
  const response = await fetch(`${BASE_URL}/api/storage/${encodeURIComponent(storageName)}/item/${encodeURIComponent(key)}`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
  });
