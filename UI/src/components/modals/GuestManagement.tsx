  ...
  return (
    ...
    >
      <div className="space-y-4">
        {guestsError && (
          <p className="text-xs text-red-400 mb-2">{guestsError}</p>
        )}
        {guestsLoading ? (
              ...
              allUsers.map((user) => (
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      handleToggleGuest(user.id, e.target.checked)
                    }
        ...
        <div className="border-t space-y-1 pt-2">
          ...
          <p className="text-xs text-gray-400 mt-2">
            As soon as a guest user is assigned to a function, every execution
            via the web will need to be authenticated with a valid Auth method.
          </p>
          <ActionButton icon="⚙️" label="Configure Guest Access" onClick={() => window.location.href = "/guest-users"} variant="secondary" />
        </div>
