...
function Modal({ isOpen, onClose, title, children, maxWidth = "md", isLoading = false }: ModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fadeIn p-4">
            <div className={`bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl border border-primary/20 text-white rounded-2xl shadow-2xl ${maxWidthClass} w-full animate-slideIn flex flex-col max-h-[90vh] relative overflow-hidden`}>
                {/* Gradient overlay */}
                ...
                {/* Content */}
                <div className="overflow-y-auto p-6 relative flex-1" style={{ scrollbarWidth: 'thin' }}>
                    {isLoading && (
