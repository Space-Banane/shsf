...
interface AccountEnvModalProps {
	isOpen: boolean;
	onClose: () => void;
	onUpdate: (env: AccountEnvVar[]) => Promise<boolean>;
	initialEnv: AccountEnvVar[];
}

function AccountEnvModal({
	isOpen,
	onClose,
	onUpdate,
	initialEnv,
}: AccountEnvModalProps) {
	const [variables, setVariables] = useState<AccountEnvVar[]>([]);
	const [loading, setLoading] = useState(false);
	const [showValues, setShowValues] = useState<{ [key: number]: boolean }>({});

	// Load initial environment variables when modal opens
	useEffect(() => {
		if (isOpen) {
			setVariables(initialEnv);
			setShowValues({});
		}
	}, [initialEnv, isOpen]);
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-blue-200/80 text-xs leading-relaxed">
								These environment variables will be injected into all your functions automatically.
								Function-specific variables will override these if they have the same name.
									...
									<div className="flex items-center gap-3">
										...
										<div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
											<input
												...
												className="w-full p-2 bg-gray-800/50 border border-gray-600/50 text-white rounded-lg focus:border-primary/50 focus:ring-2 focus:ring-primary/20 focus:outline-none transition-all duration-300 text-sm font-mono"
												value={variable.name}
												onChange={(e) =>
											...
											<div className="relative">
												<input
													...
													className="w-full p-2 pr-10 bg-gray-800/50 border border-gray-600/50 text-white rounded-lg focus:border-primary/50 focus:ring-2 focus:ring-primary/20 focus:outline-none transition-all duration-300 text-sm font-mono"
...
export default AccountEnvModal;
