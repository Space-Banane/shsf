...
function TriggerLogsModal({ isOpen, onClose, logs, isLoading = false }: TriggerLogsModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Trigger Execution Logs" maxWidth="xl" isLoading={isLoading}>
      <div className="space-y-4">
        {isLoading ? (
          <p className="text-gray-400 text-center py-4">Loading logs...</p>
        ) : sortedLogs.length > 0 ? (
          sortedLogs.map((log) => (
            <div key={log.id} className="border border-gray-700 rounded-lg bg-gray-900 overflow-hidden">
              <div 
              ...
              {expandedLogs[log.id] && (
                <div className="p-3 border-t border-gray-700">
                  {log.result && (
                    <div className="mb-3">
                      <h4 className="text-white text-sm font-medium mb-1">Payload</h4>
                      <div className="bg-gray-950 p-3 rounded max-h-60 overflow-auto">
                        <pre className="text-gray-300 text-xs font-mono whitespace-pre-wrap">
                          {JSON.stringify(JSON.parse(log.result).payload, null, 2)}
                        </pre>
                      </div>
                    </div>
                  )}
                  {log.logs && (
                    <div className="mb-3">
                      <h4 className="text-white text-sm font-medium mb-1">Output Logs:</h4>
                      <div className="bg-gray-950 p-3 rounded max-h-60 overflow-auto">
                        <pre className="text-gray-300 text-xs font-mono whitespace-pre-wrap">{log.logs}</pre>
                      </div>
                    </div>
                  )}
                  ...
                  {log.result && (
                    <div className="mb-2">
                      <h4 className="text-white text-sm font-medium mb-1">Execution Result:</h4>
                      <div className="bg-gray-950 p-3 rounded max-h-60 overflow-auto">
                        <pre className="text-gray-300 text-xs font-mono whitespace-pre-wrap">
