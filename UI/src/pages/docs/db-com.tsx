	...
	return (
		<div className="min-h-screen bg-background text-text p-8">
			<div className="max-w-4xl mx-auto">
				...
				<h1 className="text-3xl font-bold text-primary mb-2">Database Communication in Python</h1>
				<p className="mt-3 text-lg text-text/90 mb-6">
					SHSF provides a simple Python interface for fast, persistent storage and retrieval of data using the <code>_db_com.py</code> script. This lets you create, update, and fetch data from your serverless functions with minimal code.
				</p>

                <div className="mb-6 p-4 bg-yellow-900/20 border-l-4 border-yellow-400 rounded">
                    <b>Note:</b> You must install the <code>requests</code> package in your Python environment. Add it to your <code>requirements.txt</code> file.
                </div>

				<h2 className="text-2xl font-bold text-primary mt-8 mb-4">Quick Start</h2>
				<p className="mb-4 text-text/90">Import and initialize the database interface:</p>
				<pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm mb-6">
					<code>{`from _db_com import database\n\ndb = database()`}</code>
				</pre>

				<h2 className="text-xl font-bold text-primary mt-8 mb-3">Create a Storage</h2>
				<pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm mb-6">
					<code>{`db.create_storage("my_storage", purpose="For fast key-value data")`}</code>
				</pre>

				<h2 className="text-xl font-bold text-primary mt-8 mb-3">Set Data (No Expiration)</h2>
				<pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm mb-6">
					<code>{`db.set("my_storage", "username", "alice")`}</code>
				</pre>

				<h2 className="text-xl font-bold text-primary mt-8 mb-3">Set Data (With Expiration)</h2>
				<pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm mb-6">
				...
				<h2 className="text-xl font-bold text-primary mt-8 mb-3">Check if Item Exists</h2>
				<pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm mb-6">
					<code>{`if db.exists("my_storage", "username"):\n    print("User exists!")`}</code>
				</pre>

				<h2 className="text-2xl font-bold text-primary mt-10 mb-4">Best Practices & Notes</h2>
				<ul className="list-disc list-inside mb-4 text-text/90">
					<li>Use short, alphanumeric storage and key names for best performance.</li>
					<li>Always check for <code>None</code> or handle <code>DatabaseError</code> when retrieving data.</li>
