...
export const FfmpegInstallPage = () => {
		...
		<div className="min-h-screen bg-background text-text p-8">
			<div className="max-w-4xl mx-auto">
				...
				<p className="mt-3 text-lg text-text/90 mb-6">
					Enable the <b>Install FFmpeg</b> option to automatically install FFmpeg in
					your function's container environment. This is perfect for functions that
					need to process video, audio, or other media files.
				</p>
				...
				<p className="mb-4 text-text/90">
					...
					transcode, mux, demux, stream, filter, and play almost any media format.
					It's widely used for video and audio processing tasks.
				</p>
				...
				<ul className="list-disc list-inside mb-4 text-text/90">
					<li>
						When creating or updating a function, toggle the <b>Install FFmpeg</b>{" "}
						option in the advanced settings section of the modal.
					</li>
					<li>
						During container initialization, SHSF will check if FFmpeg is installed
						and install it automatically if needed.
					</li>
					<li>
						The installation happens once when the container is first created or when
				...
				<ul className="list-disc list-inside mb-4 text-text/90">
					<li>Converting video formats (e.g., MP4 to WebM, AVI to MP4).</li>
					<li>Extracting audio from video files.</li>
					<li>Generating video thumbnails or creating GIFs from videos.</li>
    ...
    return {
        ...
        "format": "webm"
					...
					<p className="text-text/80 mt-2">
						...
						format. The function receives a base64-encoded video, processes it with
						FFmpeg, and returns the converted result.
					</p>
				</div>
    ...
    return {
        ...
        "timestamp": "5s"
				...
				<p className="mb-4 text-text/90">
					When FFmpeg installation is enabled, SHSF runs the following command during
				...
				<p className="mb-4 text-text/90">
					This command checks if FFmpeg is already installed and only installs it if
					it's not found. The installation is cached in the container, so subsequent
					executions are fast.
				</p>
				...
				<ul className="list-disc list-inside mb-4 text-text/90">
					<li>
						FFmpeg operations can be CPU and memory intensive. Make sure to allocate
						sufficient resources to your function.
					</li>
					<li>
						The first run after enabling FFmpeg installation will be slower due to the
						installation process.
					</li>
					<li>
						Consider using the <code>/app</code> directory for temporary files during
						processing; <code>/tmp</code> is very experimental.
					</li>
					<li>
				...
				<div className="mt-12 p-6 bg-gradient-to-r from-purple-900/20 to-pink-900/20 border border-purple-600/30 rounded-xl">
					...
					<p className="text-text/90 mb-4">
						With FFmpeg installed, your functions can handle complex media processing
						tasks without any additional setup. Just toggle the option and start
						processing!
					</p>
				</div>
				...
				<div className="mt-10 mb-8 p-4 bg-yellow-100/10 border border-yellow-400/30 rounded-lg">
					...
					<p className="text-text/90 mb-2">
						If FFmpeg installation fails or you need to force a reinstall, you can
						remove the install marker file from the function container:
					</p>
					...
					<p className="text-text/80 mt-2">
						Replace <code>{`{FUNCTION_ID}`}</code> with your function's ID (visible in
						the URL). On next run, FFmpeg will be reinstalled automatically.
					</p>
				</div>

				<div className="mt-12 p-6 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-primary/30 rounded-xl">
					<h2 className="text-xl font-bold text-primary mb-3">
						🚀 Next Step - Go Runtime
					</h2>
					<p className="text-text/90 mb-4">
						Interested in using Go for your serverless functions? Learn how to set up
