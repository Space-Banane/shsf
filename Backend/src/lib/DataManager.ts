...
async function setupUUID() {
	...
	if (existsSync("../../.data/.uuid")) {
		return { error: "UUID already exists" };
	}

	const newUUID = crypto.randomUUID();
	try {
		writeFileSync("../../.data/.uuid", newUUID, "utf-8");
		return { uuid: newUUID };
	} catch (err) {
...
export async function getUUID() {
	...
	try {
		const uuid = readFileSync("../../.data/.uuid", "utf-8");
		return uuid;
	} catch (err) {
...
export async function getLinkStatus(): Promise<LinkStatus> {
	...
	try {
		const raw = readFileSync("../../.data/.linked", "utf-8");
...
export async function setLinkStatus(status: LinkStatus): Promise<void> {
	...
	try {
		writeFileSync("../../.data/.linked", JSON.stringify(status), "utf-8");
	} catch (err) {
...
export async function getLinkLock(): Promise<boolean> {
	...
	try {
		const raw = readFileSync("../../.data/.linkLock", "utf-8");
...
export async function setLinkLock(locked: boolean): Promise<void> {
	...
	try {
		writeFileSync("../../.data/.linkLock", JSON.stringify(locked), "utf-8");
	} catch (err) {
...
export async function getRegistrationDisabled(): Promise<boolean> {
	...
	try {
		const raw = readFileSync("../../.data/.registrationDisabled", "utf-8");
		return JSON.parse(raw) === true;
	} catch {
		return false;
	}
...
export async function setRegistrationDisabled(disabled: boolean): Promise<void> {
	...
	try {
		writeFileSync("../../.data/.registrationDisabled", JSON.stringify(disabled), "utf-8");
	} catch (err) {
