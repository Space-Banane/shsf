...
export = new fileRouter.Path("/")
	...
	.http("GET", "/api/function/{id}/git", (http) =>
		http
			.document({
				description:
					"Get git configuration for a function (never returns password)",
				tags: ["VERSION // CONTROL"] as OpenAPITags[],
				operationId: "getFunctionGitConfig",
				responses: {
					200: {
						description: "Git configuration fetched successfully",
						content: {
							"application/json": {
								schema: {
									type: "object",
									properties: {
										...
										data: {
											type: "object",
											properties: {
												git_url: { type: "string", nullable: true },
												git_username: { type: "string", nullable: true },
												git_has_credentials: { type: "boolean" },
	...
	.http("POST", "/api/function/{id}/git/clone", (http) =>
		http
			.document({
				description:
					"Clone a git repository for a function, optionally with credentials and source directory",
				tags: ["VERSION // CONTROL"] as OpenAPITags[],
				operationId: "cloneFunctionGitRepo",
				requestBody: {
					content: {
						"application/json": {
							schema: {
			...
			.onRequest(async (ctr) => {
				...
				if (sourceDir) {
					...
					logLines.push(
						`[GIT] Clone successful! Syncing source directory "${sourceDir}" → app ...`,
					);
	...
	.http("POST", "/api/function/{id}/git/pull", (http) =>
		http
			.document({
				description:
					"Manually pull latest changes from the configured git repository",
				tags: ["VERSION // CONTROL"] as OpenAPITags[],
				operationId: "pullFunctionGitRepo",
				responses: {
					200: {
			...
			.onRequest(async (ctr) => {
				...
				const result = await performGitPull(
					functionId,
					functionData.git_source_dir ?? undefined,
				);
	...
	.http("PATCH", "/api/function/{id}/git", (http) =>
		http
			.document({
				description:
					"Update git settings for a function (periodic pull, credentials, source directory)",
				tags: ["VERSION // CONTROL"] as OpenAPITags[],
				operationId: "updateFunctionGitConfig",
				requestBody: {
					content: {
						"application/json": {
							schema: {
								...
								properties: {
									git_periodic_pull: {
										type: "boolean",
										description: "Enable periodic pull",
									},
									git_pull_interval: {
										type: "number",
										description: "Pull interval in minutes",
									},
									git_username: { type: "string", description: "Git username" },
									git_password: { type: "string", description: "Git password" },
	...
	.http("DELETE", "/api/function/{id}/git", (http) =>
		http
			.document({
				description: "Remove git configuration from a function",
				tags: ["VERSION // CONTROL"] as OpenAPITags[],
				operationId: "deleteFunctionGitConfig",
				responses: {
					200: {
						description: "Git configuration removed",
						content: {
							"application/json": {
								schema: {
									type: "object",
